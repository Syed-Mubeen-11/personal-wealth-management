...
function Portfolio() {
  ...
  // FETCH REBALANCE DATA (We pass 'true' so it loads in the background for the table icons!)
  const rebalanceData = useRebalanceSuggestions(true);
        ...
        symbols.map(async (symbolItem) => {
          try {
            ...
            const fallbackRes = await fetch(
              `http://127.0.0.1:8000/stock/${symbolItem}`,
            );
            if (!fallbackRes.ok) {
  ...
  const sellAsset = async (position) => {
    ...
    const input = window.prompt(
      `Enter quantity to sell (max: ${maxQty}):`,
      maxQty,
    );
    ...
    try {
      ...
      alert(
        `Successfully sold ${qty} shares of ${position.symbol} for $${amount.toFixed(2)}`,
      );
      // Refresh all data
      loadOverview();
      loadPositions(positionsPage);
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="mt-4 text-teal-800 font-bold">
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Manage your investments and track all transactions
          </p>
        </div>
        <div className="flex gap-3">
          <button
            onClick={() => setIsDrawerOpen(true)}
            className="px-6 py-3 bg-[#1B3C53] text-white rounded-lg font-medium hover:bg-[#234C6A] transition shadow-sm"
          >
            Rebalance Portfolio
          </button>
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <PositionsTable
            ...
            onSell={sellAsset}
            rebalanceData={rebalanceData} /* WE PASSED THE PROP HERE! */
          />
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
        ...
        {marketWatch.length === 0 ? (
          <p className="text-gray-500 text-sm">
            Unable to fetch live prices right now.
          </p>
        ) : (
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  onClick={buyAsset}
                  disabled={
                    !priceData || !quantity || parseFloat(quantity) <= 0
                  }
      ...
      {/* THE DRAWER ITSELF IS AT THE BOTTOM */}
      <RebalanceDrawer
        isOpen={isDrawerOpen}
