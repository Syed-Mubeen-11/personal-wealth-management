...
export default function Register() {
  ...
  const isFormValid = form.name && isEmailValid && isPasswordValid && isConfirmValid && form.risk;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // UPGRADED ERROR HANDLING: This will tell you exactly what is wrong
      if (err.response) {
        // The backend rejected it (e.g., 400 Email already registered)
        setError(err.response.data.detail || "Server rejected the registration.");
      } else if (err.request) {
        // The frontend couldn't even reach the backend
        setError("Network error: Could not connect to the backend. Is Uvicorn running?");
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row">
      ...
      <div className="relative w-full md:w-1/2 flex items-center justify-center text-white p-10 overflow-hidden bg-gradient-to-br from-[#1B3C53] via-[#234C6A] to-[#456882]">
        <TrendingUp className="absolute top-16 left-20 opacity-10 animate-float-slow" size={80} />
        <DollarSign className="absolute bottom-20 right-24 opacity-10 animate-float-slow" size={70} />
        <BarChart3 className="absolute top-32 right-40 opacity-10 animate-float-slow" size={75} />

        <TrendingUp className="absolute bottom-32 left-36 opacity-20 animate-float-medium" size={50} />
        <DollarSign className="absolute top-52 right-16 opacity-20 animate-float-medium" size={45} />
        <BarChart3 className="absolute bottom-10 left-10 opacity-20 animate-float-medium" size={55} />

        <TrendingUp className="absolute top-10 right-10 opacity-30 animate-float-fast" size={30} />
        <DollarSign className="absolute bottom-12 right-44 opacity-30 animate-float-fast" size={28} />
        <BarChart3 className="absolute top-60 left-44 opacity-30 animate-float-fast" size={32} />
      ...
      <div className="flex-1 flex items-center justify-center bg-[#E3E3E3] p-8">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* ERROR MESSAGE */}
            {error && (
              <p className="text-red-500 text-sm text-center font-bold bg-red-50 p-3 rounded-lg border border-red-200">
                {error}
              </p>
            )}
