...
function Goals() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setFormData({ target_name: '', target_amount: '' }); // Clear form
    ...
    return (
        <div className="max-w-4xl mx-auto p-8 bg-gray-50 min-h-screen">
            <h1 className="text-3xl font-bold text-gray-800 mb-8">Financial Goals</h1>

            {/* CREATE NEW GOAL FORM */}
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-8">
                <h2 className="text-xl font-bold text-gray-800 mb-4">Set a New Goal</h2>
                <form onSubmit={handleSubmit} className="flex gap-4 items-end">
                    <div className="flex-1">
                        <label className="block text-sm font-bold text-gray-700 mb-1">Goal Name</label>
                        <input 
                            type="text" name="target_name" required
                            placeholder="e.g. Dream Car"
                            value={formData.target_name} onChange={handleChange}
                ...
                {goals.map((goal) => (
                    <div key={goal.id} className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-4">
                            <div 
                                className={`h-4 rounded-full transition-all duration-500 ${goal.percent_complete >= 100 ? 'bg-green-500' : 'bg-blue-500'}`} 
                                style={{ width: `${goal.percent_complete}%` }}
                            ></div>
                        </div>
                        <p className="text-right text-xs text-gray-500 mt-1">{goal.percent_complete}% Completed</p>
                    </div>
                ))}

                {goals.length === 0 && (
                    <p className="text-center text-gray-500 mt-4">No goals set yet. Add one above!</p>
                )}
            </div>
        </div>
    );
